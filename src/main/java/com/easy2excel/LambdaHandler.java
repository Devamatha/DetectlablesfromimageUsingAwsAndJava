...
public class LambdaHandler implements RequestHandler<S3Event, String> {
	...
	public String handleRequest(S3Event s3Event, Context context) {
		...
		for (S3EventNotification.S3EventNotificationRecord record : s3Event.getRecords()) {
			...
			DetectLabelsRequest request = new DetectLabelsRequest().withImage(image).withMaxLabels(10)
			...
			labels.forEach(label -> labelsBuilder.append(label).append(", "));
			String detectedLabels = labelsBuilder.toString();
			Item item = new Item().withPrimaryKey("RekognitionId", objectKey).withString("DetectedLabels",
					detectedLabels);
			table.putItem(item);
